...
async function main(){
    const uri = "mongodb+srv://hayagreevan:hayagreevan@cluster0.sjx3gfu.mongodb.net/?retryWrites=true&w=majority"
    const client = new MongoClient(uri);

    try{
        await client.connect();

        const productsCollection = client.db('ofds').collection('products');
        const categoriesCollection = client.db("ofds").collection("categories");
